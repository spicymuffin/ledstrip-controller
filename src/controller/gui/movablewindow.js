...
function winIsMove2(y, z) {
    if (window.storeLayer[0].currentMWJAction) {
        doActualResize(window.storeLayer, y, z, window.lyStartCoord[2]);
    } else {
...
function doActualResize(oDivs, y, z, offSet) {
    ...
    for (var x = 0; x < 2; x++) {
        ...
        if (!x && theLayer.getElementsByTagName('div')[1].style.overflow == 'auto' && navigator.product == 'Gecko' && navigator.taintEnabled) {
            ...
            if (!ppDv.tmpDiv) {
                ppDv.tmpDiv = document.createElement('div'); ppDv.appendChild(ppDv.tmpDiv);
...
function findMaxSize() {
    if (typeof (window.scrollX) == 'number') {
        var x = window.scrollX, y = window.pageYOffset;
...
function showMinWin(thisDiv, thatDiv) {
    ...
    MWJ_winZind += 2; if (thisDiv.style) {
        thisDiv.style.visibility = 'visible'; thisDiv.style.zIndex = MWJ_winZind;
